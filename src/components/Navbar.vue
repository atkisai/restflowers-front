    ...
    <div class="find_block">
      ...
      <transition name="menu">
        <div v-if="dialog_menu" class="menu">
          ...
          <Button label="Доставка" class="p-button-text" @click="$router.push('/delivery')"/>
          <Button label="Контакты" class="p-button-text" @click="$router.push('/contacts')"/>
          <!-- <a href="/franchise"><Button style="width: 100%" label="Франшиза" class="p-button-text"/></a> -->
          <Button label="О нас" class="p-button-text" @click=""/>
          <Button v-if="user!=''" label="Чат" class="p-button-text" @click="$router.push('/chat')"/>
        </div>
      </transition>

      <div class="collapse-700">
        <Button label="Доставка" class="p-button-rounded p-button-text p-button-raised" @click="$router.push('/delivery')"/>
        <Button label="Контакты" class="p-button-rounded p-button-text p-button-raised" @click="$router.push('/contacts')"/>
        <Button v-if="user!=''" label="Чат" class="p-button-rounded p-button-text p-button-raised" @click="$router.push('/chat')"/>
      </div>
...
<!--        <a :href="$router.resolve({name:'Franchise'}).href"><Button label="Франшиза" class="p-button-rounded p-button-text p-button-raised collapse-700"></Button></a>-->
        <!-- <Button @click="setFranchise" label="Франшиза" class="p-button-rounded p-button-text p-button-raised collapse-700"></Button> -->
        <Button label="О нас" class="p-button-rounded p-button-text p-button-raised collapse-700" @click=""/>
  ...
  <transition name="dialog">
    <ModalLogin v-if="dialog_login" @close="dialog_login=false" @user="((v)=>{user=v})" @user_name="((v)=>{user_name=v})" @crm_display="((v)=>{items[1].style=v})"/>
  </transition>
...
export default {
    ...
    return{
      ...
      items: [
        ...
        },
        {
          ...
          style: localStorage.getItem('crm'),
          command: () => {
            this.$router.push('/crm/products')
          }
        },
        {
          label: 'Выйти',
          ...
          command: () => {
            ...
            this.user='';
            if(this.$route.name=='Profile'){this.$router.push('/')}
            // this.$toast.add({ severity: 'warn', summary: 'Выход', detail: 'Вы вышли из профиля', life: 3000});
  ...
  computed:{
    activeAdmin(){
      if(this.admin===false){return 'false'}else{return 'true'}
    },
  },
  methods:{
    setFranchise(){
